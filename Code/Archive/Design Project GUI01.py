...
wash_end_time = datetime.now()
dry_end_time = datetime.now()
...
def controller(washing_duration, drying_duration):
    ...
    if magnetic_status():
        if washing_duration > 0:
            ...
            turn_on_water_pump()
            wash_end_time = datetime.now() + datetime.timedelta(seconds=washing_duration)
            dry_end_time = datetime.now() + datetime.timedelta(seconds=drying_duration) + datetime.timedelta(seconds=washing_duration) 
            while (status == 1) and (datetime.now() < wash_end_time):
                if magnetic_status() == False:
                    previous_status = status
                    status = 3
                    turn_off_water_pump()
                    leftover_wash_duration = (wash_end_time-datetime.now()).total_seconds()
        ...
        if drying_duration > 0:
            ...
            while (status == 2) and (datetime.now() < dry_end_time):    
                if magnetic_status() == False:
                    leftover_dry_duration = (dry_end_time-datetime.now()).total_seconds()
                    previous_status = status
                    status = 3
                    turn_off_heated_fan()
                    stateLbl.configure(text="Drying paused. Please close the door and press 'Continue' button.")
                drying_elapsed = (datetime.now() - current_time).total_seconds() 
                time.sleep(0.2)
            if status == 2:
                turn_off_heated_fan()
                status = 0
